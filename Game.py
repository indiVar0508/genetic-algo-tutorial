...
class Person:
	...
	def crossOver(parentOne, parentTwo):
		child = Person(parentTwo.gameDisplay, parentOne.windowHeight, parentTwo.windowWidth)
...
class Population:
	...
	def __init__(self, gameDisplay, windowWidth, windowHeight, mutation = 0.1, populationSize = 50, numOfMoves = 500):
		...
		self.windowWidth = windowWidth
		self.windowHeight = windowHeight
		self.players = [Person(self.gameDisplay, self.windowHeight, self.windowWidth, numOfMoves = numOfMoves) for _ in range(self.populationSize)]
		self.generation = 0
		self.matingPool = []
		self.bestFitness = 0
...
class Environment:
	...
	def __init__(self, windowWidth = 800, windowHeight = 600):
		...
		self.population = Population(self.gameDisplay, self.windowWidth, self.windowHeight,populationSize = 500, mutation = 0.03, numOfMoves = 1000)
		self.Wall = [#Maze(self.gameDisplay, 0, self.windowHeight // 3, (3*self.windowWidth) // 4, 10, self.black),\
	...
	def On(self):
		while True:
			for event in pygame.event.get():
				...
				# 		for player in self.players: player.moveUp()
				# 	elif event.key == pygame.K_DOWN: 
				# 		for player in self.players: player.moveDown()
				# 	elif event.key == pygame.K_RIGHT: 
				# 		for player in self.players: player.moveRight()
				# 	elif event.key == pygame.K_LEFT: 
				# 		for player in self.players: player.moveLeft()

			for player in self.population.players:
				if not player.dead:
					if player.gene.moves[player.geneIndex] == 0:
						if not player.dead: player.moveUp()
					elif player.gene.moves[player.geneIndex] == 1:
						if not player.dead: player.moveDown()
					elif player.gene.moves[player.geneIndex] == 2:
						if not player.dead: player.moveRight()
					elif player.gene.moves[player.geneIndex] == 3:
						if not player.dead: player.moveLeft()
			...
			self.collide()
			for player in self.population.players: player.showPerson()
			self.sayMessage(msg = ''.join(['Best Score : ', str(np.sqrt(self.population.bestFitness))]),xpos = 10, ypos = 25, color = (0, 0, 0))
			self.sayMessage(msg = ''.join(['Generation : ', str(self.population.generation)]),xpos = 10,color = (0, 0, 0))
